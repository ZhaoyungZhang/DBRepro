...
public class Distribution {
    ...
    // ★★★ 新增：全局bound信息（Stage 3使用）★★★
    private TableBoundInfo tableBoundInfo = null;
    
    // ★★★ 新增：标记此Distribution属于哪个BoundGroup ★★★
    private int boundGroupId = -1;
    ...
    public BigDecimal getOffset(long dataIndex) {
        ...
        // ★★★ 修复：如果 lastRange 为 null，说明 paraData2Probability 未初始化，返回 lastKey ★★★
        if (lastRange == null) {
            logger.warn("Distribution.getOffset: paraData2Probability 中未找到 dataIndex={}，返回 lastKey", lastPv);
            return offset2Pv.lastKey();
        }
    ...
    public long[] prepareTupleData(int size) {
        ...
        logger.info("🔗 BOUND DEBUG: Distribution.prepareTupleData() 开始处理，size={}, offset2Pv={}", size, offset2Pv);
        
        int boundIndex = 0;
        for (Map.Entry<BigDecimal, Long> pv2Offset : offset2Pv.entrySet()) {
            // 确定bound的开始offset
            int bOffset = bSize.multiply(pv2Offset.getKey()).intValue();
            logger.info("🔗 BOUND DEBUG: Distribution处理 bound {} : offset={}, dataIndex={}, bOffset={}", 
                boundIndex, pv2Offset.getKey(), pv2Offset.getValue(), bOffset);
            ...
            logger.info("🔗 BOUND DEBUG: Distribution填充 bound {} 值: 行范围 [{}, {}), 共 {} 行", 
                boundIndex, currentIndex, currentIndex + generateSize, generateSize);
        ...
        logger.info("🔗 BOUND DEBUG: Distribution.prepareTupleData() 完成，处理了 {} 个bound", boundIndex);
        return columnData;
    }
    ...
    // ★★★ 新增：获取和设置全局bound信息 ★★★
    public TableBoundInfo getTableBoundInfo() {
        return tableBoundInfo;
    }
    
    public void setTableBoundInfo(TableBoundInfo tableBoundInfo) {
        this.tableBoundInfo = tableBoundInfo;
    }
    
    // ★★★ 新增：获取和设置BoundGroup ID ★★★
    public int getBoundGroupId() {
        return boundGroupId;
    }
    
    public void setBoundGroupId(int boundGroupId) {
