...
public class PgJsonReader {
    ...
    static String readFilterInfo(StringBuilder path) {
        String filter = readContext.read(path + "['Filter']");
        if (filter == null) {
            filter = readContext.read(path + "['Recheck Cond']");
        }
        return filter;
    }

    static String readIndexCond(StringBuilder path) {
        return readContext.read(path + "['Index Cond']");
    }
    ...
    private static int readRowsCountAdaptive(StringBuilder path, String tag) {
        ...
        if (rows == null) {
            actualRows = 0;
        } else if (rows instanceof Number temp) {
            // PG JSON 中大整数常为 Long，非 Integer/Double
            actualRows = temp.doubleValue();
        } else {
            throw new UnsupportedOperationException("unexpected type for " + tag + ": " + rows.getClass().getName());
        }
        if (isGauss)
