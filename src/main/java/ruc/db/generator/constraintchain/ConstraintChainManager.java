    ...
    public void cleanConstrainChains(Map<String, List<ConstraintChain>> query2chains) {
        for (var query2ConstraintChains : query2chains.entrySet()) {
            ...
            while (constraintChainIterator.hasNext()) {
                ...
                // ★★★ 修复：保留只包含FILTER节点的约束链，因为它们包含选择率信息，用于参数实例化 ★★★
                // 只移除只包含AGGREGATE节点（且没有GroupKey）的约束链
                boolean onlyAggregateWithoutGroupKey = constraintChain.getNodes().stream().allMatch(
                        node -> node.getConstraintChainNodeType() == ConstraintChainNodeType.AGGREGATE &&
                                ((ConstraintChainAggregateNode) node).getGroupKey() == null);
                if (onlyAggregateWithoutGroupKey) {
                ...
                // 注意：只包含FILTER节点的约束链不再被移除，因为它们对于参数实例化是必要的
            }
        }
    }
